...
@property (nonatomic, copy) ATAlertInputViewTextShouldChange textShouldChangeValue;
...
+ (instancetype)showWithTitle:(NSString *)title inputConfig:(ATAlertInputViewConfig)inputConfig textShouldChangeValue:(ATAlertInputViewTextShouldChange)textShouldChangeValue handler:(ATAlertInputViewHandler)inputHandler
{
    ATAlertInputView *input = [[self alloc] initWithTitle:title inputConfig:inputConfig textShouldChangeValue:textShouldChangeValue handler:inputHandler];
    [input show];
    return input;
}

- (instancetype)initWithTitle:(NSString *)title inputConfig:(ATAlertInputViewConfig)inputConfig textShouldChangeValue:(ATAlertInputViewTextShouldChange)textShouldChangeValue handler:(ATAlertInputViewHandler)inputHandler {
    if (self = [super init]) {
        ...
        self.textShouldChangeValue = textShouldChangeValue;
        self.inputHandler = inputHandler;
        self.actionItems = @[MMItemMake(ATLocalizedString(@"Cancel", @"取消"), MMItemTypeNormal, nil),
                             MMItemMake(ATLocalizedString(@"Confirm", @"确定"), MMItemTypeHighlight, nil)
                             ];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    if (self.textShouldChangeValue) {
        return self.textShouldChangeValue(textField.tag, textField.text, [textField.text stringByReplacingCharactersInRange:range withString:string], self.confirmButton);
    }
    return YES;
}
- (void)textFieldDidEndEditing:(UITextField *)textField {
    if (self.textShouldChangeValue) {
        self.textShouldChangeValue(textField.tag, textField.text, textField.text, self.confirmButton);
    }
}
- (BOOL)textFieldShouldClear:(UITextField *)textField {
    if (self.textShouldChangeValue) {
        self.textShouldChangeValue(textField.tag, textField.text, nil, self.confirmButton);
    }
    return YES;
}
